...
func initialize(seed_value: int) -> void:
    ...
    print("[WORLD] spawn resolved at ", spawn_position, " world_height=", world_height)
    ...
    if not chunks.has(spawn_coord) and not pending.has(spawn_coord):
        ...
        if spawn_data.is_empty():
            push_error("[WORLD] FATAL: spawn chunk generation returned empty data.")
            pending.erase(spawn_coord)
            return
        print("[WORLD] spawn chunk generated bytes=", spawn_data.size())
        ...
        if spawn_chunk != null and is_instance_valid(spawn_chunk):
            ...
            var surface_count := 0
            var mesh_node := spawn_chunk.get("mesh_instance") as MeshInstance3D
            if mesh_node != null and mesh_node.mesh != null:
                surface_count = mesh_node.mesh.get_surface_count()
            if surface_count <= 0:
                push_error("[WORLD] FATAL: spawn chunk mesh built with zero surfaces.")
                return
            print("[WORLD] spawn chunk mesh surfaces=", surface_count)
            ready_emitted = true
            world_ready.emit()
            print("[WORLD] world_ready emitted")
...
func get_rendered_chunk_count() -> int:
    var count := 0
    for key in chunks:
        var chunk: Node3D = chunks[key] as Node3D
        if chunk == null or not is_instance_valid(chunk):
            continue
        var mesh_node := chunk.get("mesh_instance") as MeshInstance3D
